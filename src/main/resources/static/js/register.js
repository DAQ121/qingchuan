$(document).ready(function(){
    ...
    function nameisUsed(name) {
        if(name==null||name==""){
            ...
            return false;
        }else if(!/^[\u4E00-\u9FA5A-Za-z0-9]{2,15}$/.test(name)){
            layer.msg("用户名由2-15位字母,数字,汉字组成", {time: 2000, icon: 5, shift: 6}, function () {
            });
            return false;
        }
        //验证用户名是否可用
    ...
    $("#zc").click(function () {
        ...
        var name=$("#name").val().trim();
        var sex=$('input[id="sex"]').filter(':checked').val();
        ...
        if(password1==null||password1==''||password2==null||password2==''){
            ...
            layer.msg("密码长度要大于6位，由数字，字母组成", {time: 2000, icon: 5, shift: 6}, function () {
            });
            return false;
        }
        if(phone==null||phone==''){
        ...
        //验证通过提交表信息
        var args={"name":name,"password1":password1,"password2":password2,"code":code,"phone":phone,"time":new Date(),"sex":sex}
